...
const SimilarBooks = () => {
  ...
  const fetchSimilarBooks = useCallback(async (bookId) => {
    ...
    try {
      const apiUrl = `/api/similar-books/${bookId}?n=${recommendationCount}&include_images=true`;
        ...
        <Grid container spacing={3} alignItems="center">
          <Grid item xs={12} md={4}>
            <Autocomplete
              ...
              // Make labels unique by appending book_id to title when necessary
              getOptionLabel={(option) => {
                const title = option?.title || '';
                const count = allBooks?.filter(book => book.title === title).length || 0;
                // Only append the book_id if there are multiple books with this title
                return count > 1 ? `${title} (ID: ${option.book_id})` : title;
              }}
              ...
              renderOption={(props, option) => {
                // Ignore the key from props entirely
                let {...otherProps} = props;
                return (
                  <li key={`book-option-${option.book_id}`} {...otherProps}>
